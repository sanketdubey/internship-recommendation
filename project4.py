...
sector_filter = st.sidebar.selectbox("Filter by Sector", ["Any"] + sorted(internships['Sector'].unique()))
location_filter = st.sidebar.selectbox("Filter by Location", ["Any"] + sorted(internships['Location'].unique()))
...
if submitted:
    ...
    # filter internships dataframe
    df_filtered = internships.copy()
    ...
    if use_semantic:
        # semantic embeddings
        internship_embeddings = semantic_model.encode(df_filtered['combined'], convert_to_tensor=True)
        ...
        internship_vectors = vectorizer.fit_transform(df_filtered['combined'])
    ...
    top3 = df_filtered.sort_values(by='score', ascending=False).head(3)
...
st.info("This is an advanced prototype. Future scope: REST API with Flask, PostgreSQL DB, React frontend, "
